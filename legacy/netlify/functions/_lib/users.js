...
function adminIdSet() {
  const raw =
    trim(process.env.ADMIN_DISCORD_IDS) || "923472355924267028";
...
export async function getUserProfile(userIdRaw) {
  ...
  const now = new Date();
  const today = todayKey(now);
  const shouldBeAdmin = isAdminUserId(userId);
  if (shouldBeAdmin && trim(doc.role) !== "admin") {
    await collection.updateOne(
      { userId },
      {
        $set: {
...
export async function consumeDailyUse(userIdRaw) {
  ...
  const remaining = Math.max(0, profile.usesLeftToday - 1);
  await collection.updateOne(
